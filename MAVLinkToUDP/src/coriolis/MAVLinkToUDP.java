...
public final class MAVLinkToUDP {
	...
	public MAVLinkToUDP() throws Exception {
		...
		try {
			...
			try {
				schema = new MAVLinkSchema("common.xml");
			}
			catch (Exception err) {
				System.out.println("ERROR: Could not load Mavlink Schema: " + err.getLocalizedMessage());
				return;				
			}
		}
		...
		while (loop) {
			...
			if (msg != null) {
				FGVehicle fg_vehicle = new FGVehicle();
				if (vehicle.equals(Vehicle.QUAD)) {
					fg_vehicle.readQuad(msg);					
				} else {
					fg_vehicle.readVogi(msg);
				}
				fg_vehicle.packFG(bufferUDP); // pack the FG variables into the bufferUDP
				clientUDP.send(bufferUDP); // send the BufferUDP
				if (DEBUG_MODE)
					System.out.println(fg_vehicle);
			}
			Thread.sleep(10);
		}
